...
import { formatPrice, typeLabels } from "@/data/products";
import type { CartProductView } from "@/hooks/useCart";

interface ShoppingBagProps {
  isOpen: boolean;
  onClose: () => void;
  cartItems: CartProductView[];
  updateQuantity: (id: number, newQuantity: number) => void | Promise<unknown>;
...
const ShoppingBag = ({ isOpen, onClose, cartItems, updateQuantity, onViewFavorites }: ShoppingBagProps) => {
  ...
  const content = (
    <div className="fixed inset-0 z-[100] h-screen">
      ...
      {/* Off-canvas panel */}
      <div className="absolute right-0 top-0 h-screen w-96 bg-background border-l border-border animate-slide-in-right flex flex-col z-[101]">
        {/* Header */}
          ...
          {cartItems.length === 0 ? (
            <div className="flex-1 flex items-center justify-center">
              <p className="text-muted-foreground text-sm text-center">
                ...
                Продолжите покупки, чтобы добавить товары.
              </p>
            </div>
          ) : (
            <>
                ...
                {cartItems.map((item) => (
                  <div key={item.id} className="flex gap-4">
                    <div className="w-20 h-20 bg-muted/10 rounded-lg overflow-hidden">
                      <img 
                        src={item.image_url || item.product?.image || "/placeholder.svg"} 
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <p className="text-sm font-light text-muted-foreground">
                            {item.product_type ? typeLabels[item.product_type as keyof typeof typeLabels] : item.size || "Товар"}
                          </p>
                          <h3 className="text-sm font-medium text-foreground">{item.product_name}</h3>
                        </div>
                        <p className="text-sm font-light text-foreground">{formatPrice(item.unit_price)}</p>
                      </div>
              ...
              <div className="border-t border-border pt-6 space-y-4">
                <div className="flex justify-between items-center">
                  <span className="text-sm font-light text-foreground">Промежуточный итог</span>
                ...
                <Button 
                  ...
                  asChild
                >
                  <Link to={`/category/${CATEGORY_SLUGS.all}`}>
                    Продолжить покупки
                  </Link>
                </Button>
              </div>
