...
import { apiGetProductReviews, type ProductReview } from "@/lib/storeApi";
...
const ProductDescription = ({ product }: ProductDescriptionProps) => {
  ...
  const loadReviews = () => {
    apiGetProductReviews(product.id)
      .then(({ reviews: rows, avg_rating, review_count }) => {
        setReviews(rows);
      ...
      <div className="border-b border-border/70 lg:mb-16">
        ...
        {isReviewsOpen && (
          <div className="pb-6 pt-1 space-y-6">
            <ReviewProduct productId={product.id} onReviewSubmitted={loadReviews} />

            {!reviewsLoaded ? (
              <p className="text-sm font-light text-muted-foreground">Загрузка отзывов...</p>
            ) : reviews.length === 0 ? (
              <p className="text-sm font-light text-muted-foreground">
